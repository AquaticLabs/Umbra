...
## Trident proof of concept (2.1.0)

This working branch replaces Umbra's collection of box-style Unity windows with one injected,
Trident-inspired immediate-mode shell. The interface uses a deep-blue theme, rounded cards,
sidebar navigation, compact controls, and six pages: Player, Visuals, World, Spawn, Lobby, and Misc.

The proof-of-concept includes:

- Give/set/reset money and give/set/remove lunar coins, plus experience, health, respawn, god mode,
  infinite skills, and live stat overrides.
- Advanced aim assist with lock-on, smooth, and pseudo-silent modes; configurable FOV, range,
  smoothing, line-of-sight filtering, fire gating, priority, and aim point.
- Enemy/interactable/teleporter overlays, a crosshair, and an aim FOV guide.
- Teleporter, portal, stage, cleanup, director spawn, lobby, movement, and cooldown samples.

### Build in VS Code

1. Install Visual Studio 2022 or Build Tools with MSBuild and open this folder in VS Code.
2. Make sure Risk of Rain 2 is installed in a common Steam library. For a custom library, set
   `UMBRA_GAME_MANAGED_DIR` to `Risk of Rain 2_Data\Managed`.
3. Press `Ctrl+Shift+B`. The default task creates `bin\Debug\UmbraMenu.dll`.
4. Use **Terminal > Run Build Task > Build Umbra (Release)** for `bin\Release\UmbraMenu.dll`.

The injector contract is unchanged: namespace `UmbraMenu`, class `Loader`, method `Load`.
Press `Insert` in game to show or hide the new shell. Session-wide actions require host authority.
